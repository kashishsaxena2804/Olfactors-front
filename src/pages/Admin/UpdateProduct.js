...
const UpdateProduct = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const { data } = axios.put(
        `/api/vl/product/update-product/${id}`,
        productData
      );
      if (data?.success) {
  ...
  const handleDelete = async () => {
    try {
      const answer = window.prompt("Are You Sure want to delete this product? (yes/no)");
      if (!answer || answer.toLowerCase() !== "yes") {
        return;
      }
  
      const { data } = await axios.delete(`/api/vl/product/delete-product/${id}`);
            ...
            <div className="m-1 w-75">
              ...
              <div className="mb-3">
                {photo ? (
                  ...
                  <div className="text-center">
                    <img
                      src={`/api/vl/product/product-photo/${id}`}
                      alt="product_photo"
                      height={"200px"}
                      className="img img-responsive"
                    />
